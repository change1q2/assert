...
async function deliverEmailCode(email, code) {
  ...
  const fromEmail = process.env.RESEND_FROM_EMAIL || RESEND_FALLBACK_FROM;
  ...
  async function sendWithFrom(from) {
    const response = await fetch("https://api.resend.com/emails", {
      ...
      body: JSON.stringify({
        from: from === RESEND_FALLBACK_FROM ? RESEND_FALLBACK_FROM : `Wealth OS <${from}>`,
        to: email,
  ...
  try {
    let response = await sendWithFrom(fromEmail);

    if (response.status === 403 && fromEmail !== RESEND_FALLBACK_FROM) {
      const errorText = await response.text();
      if (errorText.includes("domain is not verified")) {
        console.warn(`[EMAIL] Domain "${fromEmail.split("@")[1]}" not verified, falling back to ${RESEND_FALLBACK_FROM}`);
        response = await sendWithFrom(RESEND_FALLBACK_FROM);
      }
    }
